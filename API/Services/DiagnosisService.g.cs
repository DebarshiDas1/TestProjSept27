...
namespace TestProjSept27.Services
{
    ...
    public interface IDiagnosisService
    {
        ...
        Task<List<Diagnosis>> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc");

        /// <summary>Adds a new diagnosis</summary>
        /// <param name="model">The diagnosis data to be added</param>
        /// <returns>The result of the operation</returns>
        Task<Guid> Create(Diagnosis model);
        ...
        /// <returns>The result of the operation</returns>
        Task<bool> Update(Guid id, Diagnosis updatedEntity);

        /// <summary>Updates a specific diagnosis by its primary key</summary>
        /// <param name="id">The primary key of the diagnosis</param>
        /// <param name="updatedEntity">The diagnosis data to be updated</param>
        /// <returns>The result of the operation</returns>
        Task<bool> Patch(Guid id, JsonPatchDocument<Diagnosis> updatedEntity);
    ...
    public class DiagnosisService : IDiagnosisService
    {
        ...
        public DiagnosisService(TestProjSept27Context dbContext, IFieldMapperService mapper)
        {
        ...
        public async Task<dynamic> GetById(Guid id, string fields)
        {
            ...
            string[] navigationProperties = ["MedicalRecordId_MedicalRecord"];
        ...
        public async Task<List<Diagnosis>> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            var result = await GetDiagnosis(filters, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return result;
        }
        ...
        /// <param name="model">The diagnosis data to be added</param>
        /// <returns>The result of the operation</returns>
        public async Task<Guid> Create(Diagnosis model)
        {
            model.Id = await CreateDiagnosis(model);
            return model.Id;
        }
        ...
        public async Task<bool> Update(Guid id, Diagnosis updatedEntity)
        {
            await UpdateDiagnosis(id, updatedEntity);
            return true;
        }
        ...
        /// <returns>The result of the operation</returns>
        /// <exception cref="Exception"></exception>
        public async Task<bool> Patch(Guid id, JsonPatchDocument<Diagnosis> updatedEntity)
        {
            await PatchDiagnosis(id, updatedEntity);
            return true;
        }
        ...
        #region
        private async Task<List<Diagnosis>> GetDiagnosis(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            if (pageSize < 1)
            {
            ...
            var query = _dbContext.Diagnosis.IncludeRelated().AsQueryable();
        ...
        private async Task<Guid> CreateDiagnosis(Diagnosis model)
        {
        ...
        private async Task UpdateDiagnosis(Guid id, Diagnosis updatedEntity)
        {
            _dbContext.Diagnosis.Update(updatedEntity);
            await _dbContext.SaveChangesAsync();
        }

        private async Task<bool> DeleteDiagnosis(Guid id)
        {
        ...
        private async Task PatchDiagnosis(Guid id, JsonPatchDocument<Diagnosis> updatedEntity)
        {
