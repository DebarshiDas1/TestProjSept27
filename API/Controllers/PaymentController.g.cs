...
namespace TestProjSept27.Controllers
{
    ...
    public class PaymentController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] Payment model)
        {
            ...
            var id = await _paymentService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _paymentService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _paymentService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _paymentService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] Payment updatedEntity)
        {
            ...
            var status = await _paymentService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("Payment", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<Payment> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _paymentService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}