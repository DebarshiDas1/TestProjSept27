...
{
    /// <summary>
    /// Controller responsible for managing diagnosis related operations.
    /// </summary>
    /// <remarks>
    ...
    public class DiagnosisController : BaseApiController
    {
        ...
        /// <summary>
        /// Initializes a new instance of the DiagnosisController class with the specified context.
        /// </summary>
        /// <param name="idiagnosisservice">The idiagnosisservice to be used by the controller.</param>
        public DiagnosisController(IDiagnosisService idiagnosisservice)
        {
            _diagnosisService = idiagnosisservice;
        }
        ...
        public async Task<IActionResult> Post([FromBody] Diagnosis model)
        {
            model.TenantId = TenantId;
            model.CreatedOn = DateTime.UtcNow;
            model.CreatedBy = UserId;
            var id = await _diagnosisService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _diagnosisService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _diagnosisService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _diagnosisService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] Diagnosis updatedEntity)
        {
            ...
            var status = await _diagnosisService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("Diagnosis", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<Diagnosis> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _diagnosisService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}