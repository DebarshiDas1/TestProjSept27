...
namespace TestProjSept27.Controllers
{
    ...
    public class MedicationController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] Medication model)
        {
            model.TenantId = TenantId;
            model.CreatedOn = DateTime.UtcNow;
            model.CreatedBy = UserId;
            var id = await _medicationService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _medicationService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _medicationService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _medicationService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] Medication updatedEntity)
        {
            ...
            var status = await _medicationService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("Medication", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<Medication> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _medicationService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}